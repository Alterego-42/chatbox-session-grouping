...
const sessionMetaKeys = [
  ...
  'backgroundImage',
  'type',
  // Session-grouping fork: keep group membership / system flag / manual order on the meta record,
  // otherwise updateSession() would silently drop a session out of its group.
  'groupId',
  'system',
  'sortIndex',
...
export function projectSessionMeta(session: SessionMeta): SessionMeta {
  return {
    ...
    ...(Object.hasOwn(session, 'groupId') ? { groupId: session.groupId } : {}),
    ...(Object.hasOwn(session, 'system') ? { system: session.system } : {}),
    ...(Object.hasOwn(session, 'sortIndex') ? { sortIndex: session.sortIndex } : {}),
  }
}
