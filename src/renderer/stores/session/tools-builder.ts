...
export interface BuildToolsOptions {
  /** Session id; when it equals the reserved AI-manager session id, the session-management toolset is exposed. */
  sessionId?: string
  webBrowsing: boolean
...
`
  ...
  // operations + summaries). Lazy-imported so this module doesn't pull renderer-only session
  // stores at load time (keeps the tool-builder importable in node test env).
