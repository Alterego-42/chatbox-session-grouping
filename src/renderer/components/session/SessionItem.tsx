...
export interface Props {
  ...
  /** In the virtual Starred view, hide move/archive/reorder — only un-pinning removes a session from there. */
  restricted?: boolean
  /** Rendered inside a dnd-kit DragOverlay (fork): lifted look, no interactions. */
  isOverlay?: boolean
}
...
function SessionItem(props: Props) {
  ...
  const groupMenuItems: ActionMenuItemProps[] = [
    {
      text: t('Edit') || '',
      icon: IconEdit,
      onClick: async () => {
        await NiceModal.show('session-settings', {
          session: await rendererApplication.sessionQueryBridge.getSession(session.id),
        })
      },
    },
    ...(restricted
  ...
  const content = (
    ...
    >
      ...
      <Flex
        gap={2}
        className={clsx(
          isSmallScreen || renaming ? 'hidden' : desktopMenuOpened ? 'flex' : 'group-hover/session-item:flex hidden'
        )}
      >
